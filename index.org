...
Babel calls the function =org-babel-execute-src-block=.  Inline [[http://orgmode.org/worg/org-contrib/babel/intro.html][source code]] can be written as =src_lang{code}=.  For example, highlight this source code =src_emacs-lisp{(+ 1 1)}= and press *C-c C-c*, it will result to =2=.  A different way to interpret emacs-lisp source code inline is to write the expression as an orgmode link using the prefix =elisp(<code expression>)= and evaluate the source code using *C-c C-o*, just like you open a link. For example [[elisp:(org-agenda)]].
